...
void register_with_naming_server(const char *ip, int port, int storage_port, const char *storage_server_ip, const char *file_name)
{
    ...
    // Send initial information to Naming Server
    ...
    send(sock, "Sending the list of files", strlen("Sending the list of files"), 0);

    // Send file list in chunks
    size_t file_list_len = strlen(file_list);
    size_t bytes_sent = 0;
    while (bytes_sent < file_list_len)
    {
        size_t chunk_size = (file_list_len - bytes_sent > BUFFER_SIZE) ? BUFFER_SIZE : (file_list_len - bytes_sent);
        if (send(sock, file_list + bytes_sent, chunk_size, 0) == -1)
        {
            perror("Failed to send file list chunk");
            close(sock);
            return;
        }
        bytes_sent += chunk_size;
        sleep(1); // Optional: add delay between chunks if needed
    }
...

{
    ...
    printf("Finished sending audio file: %s\n", file_path);
}
